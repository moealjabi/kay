...
const {
  providers: { createLocalKayonaSourceProvider },
} = strapiDataTransfer;
 ...
 * Export command.
 *
 * It transfers data from a local Kayona instance to a file
 *
 ...
 */
const createSourceProvider = (strapi: Core.Kayona) => {
  return createLocalKayonaSourceProvider({
    async getKayona() {
      return strapi;
    },
  });
