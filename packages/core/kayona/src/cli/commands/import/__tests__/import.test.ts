...
jest.mock('../../../utils/data-transfer', () => {
  return {
    ...
    createKayonaInstance: jest.fn().mockReturnValue({
      telemetry: {
        send: jest.fn(),
      },
      destroy: jest.fn(),
  ...
  return {
    ...
    strapi: {
      ...
      providers: {
        ...
        createLocalKayonaDestinationProvider: jest
          .fn()
          .mockReturnValue({ name: 'testKayonaDest', type: 'destination', getMetadata: jest.fn() }),
      },
    },
    engine: {
      ...actual.engine,
...
describe('Import', () => {
  ...
  it('creates providers with correct options ', async () => {
    ...
    // strapi options
    expect(strapiDataTransfer.providers.createLocalKayonaDestinationProvider).toHaveBeenCalledWith(
    ...
    expect(engineDataTransfer.createTransferEngine).toHaveBeenCalledWith(
      ...
      expect.objectContaining({ name: 'testKayonaDest' }),
      expect.objectContaining({
