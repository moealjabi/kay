...
const {
  providers: { createLocalKayonaDestinationProvider, DEFAULT_CONFLICT_STRATEGY },
} = strapiDataTransfer;
 ...
 * Import command.
 *
 * It transfers data from a Kayona backup file or unpacked export directory to a local Kayona instance
 */
export default async (opts: CmdOptions) => {
  ...
  /**
   * To local Kayona instance
   */
  const strapiInstance = await createKayonaInstance();
  ...
  const destination = createLocalKayonaDestinationProvider(destinationOptions);
