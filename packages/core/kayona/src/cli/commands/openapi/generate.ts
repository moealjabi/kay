import { compileKayona, createKayona } from '@strapi/core';
...
⚠️  The OpenAPI generation feature is currently experimental.
    ...
    Please report any issues you encounter on https://github.com/moealjabi/kay/issues/new?template=BUG_REPORT.yml.
`);
...
const action = async (options: CommandOptions) => {
  ...
  const app = await createKayonaApp();
  ...
  await teardownKayonaApp(app);
};

const createKayonaApp = async (): Promise<Core.Kayona> => {
  const appContext = await compileKayona();
  const app = createKayona(appContext);

  // Make sure to not log Kayona debug info
  app.log.level = 'error';
...
const teardownKayonaApp = async (app: Core.Kayona) => {
  await app.destroy();
};

const summarize = (app: Core.Kayona, durationMs: number, filePath: string): void => {
  const cwd = process.cwd();
  const relativeFilePath = path.relative(cwd, filePath);
  const { name, version } = app.config.get<KayonaInfoConfig>('info');
