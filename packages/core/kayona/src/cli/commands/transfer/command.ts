  ...
  return (
    createCommand('transfer')
      ...
      .addOption(
        new Option(
          '--from <sourceURL>',
          `URL of the remote Kayona instance to get data from`
        ).argParser(parseURL)
      )
      .addOption(new Option('--from-token <token>', `Transfer token for the remote Kayona source`))
      .addOption(
        new Option(
          '--to <destinationURL>',
          `URL of the remote Kayona instance to send data to`
        ).argParser(parseURL)
      )

      .addOption(
        new Option('--to-token <token>', `Transfer token for the remote Kayona destination`)
      )
      .addOption(
        new Option(
          '--no-checksums',
      ...
      .hook(
        ...
        ifOptions(
          ...
          async (thisCommand) => {
            ...
            const hasEnvUrl = process.env.KAYONA_TRANSFER_URL;
            const hasEnvToken = process.env.KAYONA_TRANSFER_TOKEN;

            const logDocumentation = () => {
              console.info(
                'ℹ️  Data transfer documentation: https://docs.kayona.io/dev-docs/data-management/transfer'
              );
            };
            ...
            const logEnvironmentVariables = () => {
              if (!hasEnvUrl && !hasEnvToken) {
                ...
                console.info(
                  '   → Add KAYONA_TRANSFER_URL and KAYONA_TRANSFER_TOKEN environment variables to make the transfer process faster for future runs'
                );
                return;
              }
              ...
              if (hasEnvUrl) {
                console.info(
                  `   → Environment KAYONA_TRANSFER_URL (${hasEnvUrl}) will be used as the transfer URL`
                );
              }

              if (hasEnvToken) {
                console.info(
                  '   → Environment KAYONA_TRANSFER_TOKEN value will be used as the transfer token'
                );
              }
              ...
              const { dir } = await inquirer.prompt([
                {
                  ...
                  choices: [
                    { name: 'Pull data from remote Kayona to local', value: 'from' },
                    { name: 'Push local data to remote Kayona', value: 'to' },
                  ],
                },
              ]);
            ...
            const determineUrl = async (direction: 'from' | 'to') => {
              ...
              if (process.env.KAYONA_TRANSFER_URL) {
                return new URL(process.env.KAYONA_TRANSFER_URL);
              }
              ...
              const answer = await inquirer.prompt([
                {
                  type: 'input',
                  name: 'remoteUrl',
                  message: `Enter the URL of the remote Kayona instance to ${direction === 'from' ? 'get data from' : 'send data to'}:`,
                  default: process.env.KAYONA_TRANSFER_URL,
            ...
            const determineToken = async (direction: 'from' | 'to') => {
              ...
              if (process.env.KAYONA_TRANSFER_TOKEN) {
                return process.env.KAYONA_TRANSFER_TOKEN;
              }
              ...
              const answer = await inquirer.prompt([
                {
                  type: 'password',
                  name: 'token',
                  message: `Enter the transfer token for the remote Kayona ${direction === 'from' ? 'source' : 'destination'}:`,
                  default: process.env.KAYONA_TRANSFER_TOKEN,
                  validate(input: string) {
                    if (!input?.length) {
          ...
          async (thisCommand) => {
            ...
            if (!thisCommand.opts().fromToken) {
              ...
              const answers = await inquirer.prompt([
                {
                  type: 'password',
                  message: 'Please enter your transfer token for the remote Kayona source',
                  name: 'fromToken',
                },
              ]);
            ...
            await getCommanderConfirmMessage(
              'The transfer will delete all the local Kayona assets and its database. Are you sure you want to proceed?',
          ...
          async (thisCommand) => {
            ...
            if (!thisCommand.opts().toToken) {
              ...
              const answers = await inquirer.prompt([
                {
                  type: 'password',
                  message: 'Please enter your transfer token for the remote Kayona destination',
                  name: 'toToken',
                },
              ]);
            ...
            await getCommanderConfirmMessage(
              'The transfer will delete existing data from the remote Kayona! Are you sure you want to proceed?',
              { failMessage: 'Transfer process aborted' }
            )(thisCommand);
          }
        )
