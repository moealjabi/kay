...
/**
 * Helper functions for the Kayona CLI
 */
...
const assertCwdContainsKayonaProject = (name: string) => {
  const logErrorAndExit = () => {
    console.log(
      `You need to run ${chalk.yellow(
        `strapi ${name}`
      )} in a Kayona project. Make sure you are in the right directory.`
    );
    process.exit(1);
  };
...
const runAction =
  ...
  (...args: unknown[]) => {
    assertCwdContainsKayonaProject(name);
...
export {
  ...
  runAction,
  assertCwdContainsKayonaProject,
