...
const command = () => {
  return (
    createCommand('import')
      .description('Import data from file to Kayona')
      .allowExcessArguments(false)
      .requiredOption(
        '-f, --file <file>',
        'path to a Kayona export (.tar[.gz][.enc]) or to an unpacked export directory'
      )
      .addOption(
        new Option(
          '-k, --key <string>',
      ...
      .hook('preAction', (thisCommand) => {
        ...
        if (extname(file) !== '.tar') {
          exitWith(
            1,
            `The file '${opts.file}' does not appear to be a valid Kayona data file. Use a path ending in .tar[.gz][.enc], or an existing directory that contains an unpacked export (e.g. metadata.json).`
          );
        }
      })
      .hook(
