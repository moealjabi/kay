...
const action = async () => {
  ...
  console.log(`${chalk.green('Successfully opted into and enabled Kayona telemetry')}`);
  process.exit(0);
};
...
const command: KayonaCommand = () => {
  return createCommand('telemetry:enable')
    .description('Enable anonymous telemetry and metadata sending to Kayona analytics')
