...
jest.mock('../../../utils/data-transfer', () => {
  return {
    ...
    createKayonaInstance: jest.fn(async () => ({
      config: {
        get: jest.fn(),
      },
      telemetry: {
  ...
  return {
    ...
    strapi: {
      ...
      providers: {
        ...
        createLocalKayonaSourceProvider: jest.fn().mockReturnValue({ name: 'testLocalSource' }),
        createLocalKayonaDestinationProvider: jest.fn().mockReturnValue({ name: 'testLocalDest' }),
        createRemoteKayonaSourceProvider: jest.fn().mockReturnValue({ name: 'testRemoteSource' }),
        createRemoteKayonaDestinationProvider: jest
          .fn()
...
describe('Transfer', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    (dataTransferUtils.createKayonaInstance as jest.Mock).mockImplementation(async () => ({
      config: {
        get: jest.fn(),
      },
      telemetry: {
  ...
  it('exits with error when no --to or --from is provided', async () => {
    ...
    expect(
      mockDataTransfer.strapi.providers.createRemoteKayonaDestinationProvider
  ...
  describe('--to', () => {
    ...
    it('uses local Kayona source when from is not specified', async () => {
      await expectExit(0, async () => {
        await transferAction({
          from: undefined,
          to: destinationUrl,
      ...
      expect(console.error).not.toHaveBeenCalled();
      expect(mockDataTransfer.strapi.providers.createLocalKayonaSourceProvider).toHaveBeenCalled();
      expect(
        mockDataTransfer.strapi.providers.createRemoteKayonaDestinationProvider
      ).toHaveBeenCalled();
    });
  ...
  describe('--from', () => {
    it('exits with error when auth is not provided', async () => {
      ...
      expect(
        mockDataTransfer.strapi.providers.createRemoteKayonaSourceProvider
      ).not.toHaveBeenCalled();
    });
    ...
    it('passes server.transfer.remote.assetIdleTimeoutMs to remote source provider as streamTimeout', async () => {
      (dataTransferUtils.createKayonaInstance as jest.Mock).mockImplementationOnce(async () => ({
        config: {
          get: (key: string) =>
    ...
    it('uses local Kayona destination when to is not specified', async () => {
      await expectExit(0, async () => {
        await transferAction({
          to: undefined,
          from: sourceUrl,
      ...
      expect(console.error).not.toHaveBeenCalled();
      expect(
        mockDataTransfer.strapi.providers.createLocalKayonaDestinationProvider
      ).toHaveBeenCalled();
      expect(mockDataTransfer.strapi.providers.createRemoteKayonaSourceProvider).toHaveBeenCalled();
    });
  });
