...
const {
  providers: {
    createRemoteKayonaDestinationProvider,
    createLocalKayonaSourceProvider,
    createLocalKayonaDestinationProvider,
    createRemoteKayonaSourceProvider,
  },
} = strapiDataTransfer;
 ...
 * Transfer command.
 *
 * Transfers data between local Kayona and remote Kayona instances
 */
export default async (opts: CmdOptions) => {
  ...
  const strapi = await createKayonaInstance();
  const checksumsEnabled = opts.checksums !== false;
  let source;
  let destination;

  // if no URL provided, use local Kayona
  if (!opts.from) {
    source = createLocalKayonaSourceProvider({
      getKayona: () => strapi,
    });
  }
  // if URL provided, set up a remote source provider
    ...
    source = createRemoteKayonaSourceProvider({
      getKayona: () => strapi,
      url: opts.from,
      auth: {
        type: 'token',
  ...
  // if no URL provided, use local Kayona
  if (!opts.to) {
    destination = createLocalKayonaDestinationProvider({
      getKayona: () => strapi,
      strategy: 'restore',
  ...
  // if URL provided, set up a remote destination provider
    ...
    destination = createRemoteKayonaDestinationProvider({
      url: opts.to,
      auth: {
        type: 'token',
        token: opts.toToken,
