...
const setSignalHandler = async (
  ...
  signals.forEach((signal) => {
    // We specifically remove ALL listeners because we have to clear the one added in Kayona bootstrap that has a process.exit
    // TODO: Ideally Kayona bootstrap would not add that listener, and then this could be more flexible and add/remove only what it needs to
    process.removeAllListeners(signal);
    process.on(signal, handler);
  });
};

const createKayonaInstance = async (opts: { logLevel?: string } = {}): Promise<Core.Kayona> => {
  try {
    const appContext = await compileKayona();
    const app = createKayona({ ...opts, ...appContext });

    app.log.level = opts.logLevel || 'error';
    return await app.load();
  } catch (error) {
    if (error instanceof Error && 'code' in error && error.code === 'ECONNREFUSED') {
      throw new Error('Process failed. Check the database connection with your Kayona project.');
    }
...
const getDiffHandler = (
  ...
  return async (
    ...
    setSignalHandler(async () => {
      await abortTransfer({ engine, strapi: strapi as Core.Kayona });
    ...
    setSignalHandler(() => abortTransfer({ engine, strapi: strapi as Core.Kayona }));
...
const getAssetsBackupHandler = (
  ...
  return async (
    ...
    console.warn(
      'The backup for the assets could not be created inside the public directory. Ensure Kayona has write permissions on the public directory.'
    );
    const confirmed = await confirmMessage(
...
type RestoreConfig = NonNullable<
  strapiDataTransfer.providers.ILocalKayonaDestinationProviderOptions['restore']
>;
...
const parseRestoreFromOptions = (
  ...
  const restoreConfig: strapiDataTransfer.providers.ILocalKayonaDestinationProviderOptions['restore'] =
    {
